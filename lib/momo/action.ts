...
import { rateDefault } from "../formatPrice";

export async function createMomoCheckout(courseId: string, userId: string) {
  try {
    ...
    const priceInVND = course.price * rateDefault;
    ...
    const body = {
      price_data: {
        ...
        unit_amount: priceInVND,
      },
      quantity: 1,
      mode: "payment",
      success_url: `${baseUrl}/courses/${slug.current}/callback`,
      cancel_url: `${baseUrl}/courses/${slug.current}/callback`,
      metadata: {
        courseId: course._id,
        userId: userId,
      },
    ...
    const session = await fetch(`${baseUrl}/api/momo/webhook`, {
      method: "POST",
